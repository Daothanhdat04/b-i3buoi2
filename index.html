<!DOCTYPE html>
<html lang="vi">
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>Trang Web của Đào Thành Đạt</title>
    <style>
       body {
            font-family: Arial, sans-serif;
            line-height: 1.6;
            margin: 20px;
            margin-left: 250px;
            margin-right: 250px;
            background: #ffffff;
            color: #000000
        }
        h1 {
            background-color: #yellow;
            margin-left: 100px;
            Text-align: center;
        }
        h2 {
            color: #444;
            margin-top: 20px;
        }
        h3 {
            font-size: 16pt;
            color: #FF0000;
        }
        ol {
            border-style: groove;
            border-width: 4px;
            border-color: blue;
        }
        ul {
            margin: 10px 0;
        }
        code {
            background: #f4f4f4;
            border: 1px solid #ddd;
            padding: 2px 4px;
            border-radius: 4px;
        }
        pre {
            background: #f4f4f4;
            border: 1px solid #ddd;
            padding: 10px;
            border-radius: 4px;
            overflow-x: auto;
        }
    </style>
</head>
<body>
    <h1>CHÀO MỪNG ĐẾN VỚI TRANG WEB CỦA ĐÀO THÀNH ĐẠT</h1>

    <p class="highlight">ĐÂY LÀ THÔNG TIN CỦA TÔI</p>
    <p class="bordered">HÃY LƯỚT XUỐNG ĐỂ XEM CHI TIẾT</p>

    <img src="data:image/jpeg;base64," alt=" Ảnh Thí Nghiệm">

    <h2>Giới thiệu</h2>
    <ol>
        <li><a href="https://www.facebook.com/daothanhdat2807" target="_blank">facebook cá nhân</a></li>
        <li><a href="https://locket.camera/invites/lhJpDT9HO6PKfX9jqQzhzhgkQWC2875d88268c129fab?type=Link&variant=1002&social_title=Th%C3%AAm+t%C3%B4i+tr%C3%AAn+Locket+%F0%9F%92%9B&social_desc=T%C3%B4i+mu%E1%BB%91n+th%C3%AAm+b%E1%BA%A1n+v%C3%A0o+M%C3%A0n+h%C3%ACnh+ch%C3%ADnh+c%E1%BB%A7a+t%C3%B4i+qua+Locket.+Ch%E1%BA%A1m+v%C3%A0o+li%C3%AAn+k%E1%BA%BFt+%C4%91%E1%BB%83+ch%E1%BA%A5p+nh%E1%BA%ADn.&social_img=https://firebasestorage.googleapis.com:443/v0/b/locket-img/o/users%252FlhJpDT9HO6PKfX9jqQzhzhgkQWC2%252Fpublic%252Finvite_link_previews%252F1719829748.599421.webp?alt%3Dmedia%26token%3Dde754fc5-35b6-4d28-a256-b718f2b939d8&lang=vi" target="_blank">locket cá nhân</a></li>
        <li><a href="https://www.instagram.com/" target="_blank">instagram cá nhân</a></li>
    </ol>

    <h2>Danh Sách Không Thứ Tự</h2>
    <ul>
        <li>Mục A</li>
        <li>Mục B</li>
        <li>Mục C</li>
    </ul>

    <h2>Bảng Thông Tin</h2>
    <table border="1">
        <thead>
            <tr>
                <th>STT</th>
                <th>Nội dung</th>
                <th>Link</th>
            </tr>
        </thead>
        <tbody>
            <tr>
                <td>1</td>
                <td>mozilla firebox.</td>
                <td><a href="https://www.mozilla.org/en-US/firefox/" target="_blank">mozilla firebox.</a></td>
            </tr>
            <tr>
                <td>2</td>
                <td>google chrome.</td>
                <td><a href="https://www.google.com/chrome/index.html" target="_blank">google chrome.</a></td>
            </tr>
            <tr>
                <td>3</td>
                <td>opera</td>
                <td><a href="https://www.opera.com/vi/download" target="_blank">opera.</a></td>
            </tr>
            <tr>
                <td>4</td>
                <td>internet explorer.</td>
                <td><a href="https://support.microsoft.com/vi-vn/window" target="_blank">internet explorer.</a></td>
                
            </tr>
        </tbody>
    </table>
</body>
</html>
